...
from OCR_windows_Classes import OCR_pytesseract,OCR_my_model
from ImageEditor_Class import ImageEditor
from Draw_class import Draw
from Preprocessing_Image_Class import Preprocessing
from Functions import display_screen_size
from Frame_Class import Frame_Mail
...
class Menu(ttk.Frame):
    ...
    def open_Image_window(self):
        second_view = ImageEditor(self, "Image Editor",'rescaling')
        return
...
class Mail_sender(tk.Toplevel):
    def __init__(self,parent):
        super().__init__(parent)
        self.geometry(f"{x}x{y+60}+{w}-{h+250}")
